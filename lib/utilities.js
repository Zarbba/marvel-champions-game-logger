...
function processRedSkullPlayerFormData(input, array) {
    ...
    input.rescuedAllyCards = convertToArray(input.rescuedAllyCards)
    input.expertObligationCards = convertToArray(input.expertObligationCards)
    ...
    input.remainingHitPoints.forEach( (player, i) => {
        ...
        array[i].rescuedAllyCards = input.rescuedAllyCards[i] ? input.rescuedAllyCards[i] : []
        array[i].expertObligationCards = input.expertObligationCards[i] ? input.expertObligationCards[i] : []
        //FIXME - These arrays do not assign correctly. Need to figure out how to get them to stick to unique players.
...
async function processRedSkullFormData(input) {
    ...
    const players = processPlayerFormData(input)
