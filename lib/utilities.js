...
function processRedSkullPlayerFormData(input, array) {
    ...
    input.remainingHitPoints.forEach( (player, i) => {
        ...
        if (input.techCard[i] !== ``) {array[i].techCard = input.techCard[i]}
...
function processCampaignFormData(input, owner) {
    ...
    const games = input.games ? convertToArray(input.games).filter(game => game !== ``) : []
...
async function paginateModel(modelName, currentPage, gamesPerPage, sorter, order) {
    const paginatedResults = await mongoose.model(modelName).find({}, null, {limit: gamesPerPage, skip: (currentPage - 1) * gamesPerPage}).populate(`owner`).sort({[sorter]: `${order}`})
    const totalPages = Math.ceil((await mongoose.model(modelName).countDocuments()) / gamesPerPage)
